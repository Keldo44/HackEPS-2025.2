...
function predictStereo() {
  ...
  if (pointDisplay1.dataset.x && pointDisplay2.dataset.x) {
    ...
    Y = pt3D.Y.toFixed(3);
    Z = pt3D.Z.toFixed(3);
    stereoDisplay.innerText = `Stereo XYZ: X=${pt3D.X.toFixed(3)}m, Y=${pt3D.Y.toFixed(3)}m, Z=${pt3D.Z.toFixed(3)}m`;
    console.log(`Stereo XYZ: X=${pt3D.X.toFixed(3)}m, Y=${pt3D.Y.toFixed(3)}m, Z=${pt3D.Z.toFixed(3)}m`)
  }
